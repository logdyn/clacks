...
module.exports = class RoutingController {
    ...
    removeWebsocket(socket)
    {
        ...
        if (websockets.length = 1 && websockets[0] === socket) {
            ...
            httpSessionMap.set(websockets.filter(ws => ws === socket));
        }
    }

    sendToUsername(payload, username) {
    ...
    sendToHttpSession(payload, httpSessionId, username) {
        ...
        if (typeof username === 'undefined') {
            let httpSessionIterator = this.userMap.values();
            let iteratorResult = httpSessionIterator.next();
            while(!iteratorResult.done)
            {
                let contents = iteratorResult.value.get(httpSessionId);
                if (typeof contents !== 'undefined') {
                    wsArray = contents;
                    break;
                }
                iteratorResult = httpSessionIterator.next();
            }
        } else {
            let userHttpSessionMap = this.userMap.get(username);
            wsArray = userHttpSessionMap.get(httpSessionId);
        }
        wsArray.forEach(ws => ws.send(payload));
    }
    ...
    sendToAll(payload)
    {
        this.userMap.forEach(httpSessionMap => {
            httpSessionMap.forEach(wsArray => {
                wsArray.forEach(ws => ws.send(paload));
            });
        });
    }
};